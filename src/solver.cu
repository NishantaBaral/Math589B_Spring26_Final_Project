...
State dynamics(State s, double alpha) {
    ...
    double phi = s.phi;
    double lambda1 = s.lambda1;
    double lambda2 = s.lambda2;

    double sin_theta = sin(theta);
    double cos_theta = cos(theta);

    double u = -lambda2 * cos_theta;
    ...
    ds.phi =
        sin_theta
        - alpha * phi
        - lambda2 * cos_theta * cos_theta;

    ds.lambda1 =
        -sin_theta
        - lambda2 * cos_theta
        - lambda2 * lambda2 * sin_theta * cos_theta;

    ds.lambda2 =
        -phi - lambda1 + alpha * lambda2;
...
void residual(
    ...
    State final_state = integrate_rk4(
        theta0,
        phi0,
        lambda1_0,
        lambda2_0,
    ...
    *r1 = final_state.theta;
    *r2 = final_state.phi;
}

__device__
int solve_two_by_two(
    double a,
    double b,
    double c,
    double d,
...
Result solve(double theta, double phi) {
    ...
    cudaMalloc((void**)&device_result, sizeof(Result));
